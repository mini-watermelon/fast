 ...
 * @jdk 1.8
 */
public class OracleParseDBXml implements ParseDBXml{

    private static final Logger LOGGER = Logger.getLogger(OracleParseDBXml.class);
    ...
    public List<Module> parse(String directory) {
        ...
        //2. 解析文件
        ParseDom parseDom = new OracleParseDom();
        for (File file: fileList) {
            Module module = parseDom.parseDom(file);
            moduleList.add(module);
        }

        // XML文件解析完毕，得到的是一个module的集合。接下来要对这个集合进行操作
        return moduleList;
    }
}
